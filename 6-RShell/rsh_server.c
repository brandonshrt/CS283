...
int start_server(char *ifaces, int port, int is_threaded){
    ...
    if (is_threaded) 
    {
        printf("Threading not implemented...\n");
    }
...
int process_cli_requests(int svr_socket){
    ...
    while(1){
        ...
        if (rc == OK_EXIT) 
        {
            break;  // Stop server on stop-server command
        }
    }
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        while (!is_last_chunk) 
        {
            ...
            if (io_size <= 0) 
            {
                last_rc = (io_size == 0) ? OK : ERR_RDSH_COMMUNICATION;
                free(io_buff);
                return rc;  // Client disconnected or error
            }
            for (int i = 0; i < io_size && cmd_len < SH_CMD_MAX - 1; i++) 
            {
                if (io_buff[i] == '\0') 
                {
                    is_last_chunk = 1;
        ...
        //cmd_list = {0};
        cmd_rc = build_cmd_list(cmd_buffer, &cmd_list);
        if (cmd_rc != OK) {
            char err_msg[256];
        ...
        if (cmd_list.num == 1) 
        {
            ...
            switch (cmd_type) 
            {
                ...
                case BI_CMD_CD:
                    ...
                    continue;
                case BI_NOT_BI:
                    break;  // Proceed to execute external command
                case BI_CMD_DRAGON:
    ...
    for (int i = 0; i < clist->num; i++) {
        ...
        } else if (pids[i] == 0) 
        {
            ...
            if (bi_cmd != BI_NOT_BI) 
            {
                ...
                fprintf(stderr, "Error: Built-in command '%s' in pipeline\n", clist->commands[i].argv[0]);
                exit(EXIT_FAILURE);
            }
            ...
            if (clist->num == 1) 
            {
                // Single command: STDIN, STDOUT, STDERR to socket
            ...
            } else 
            {
                if (i == 0) 
                {
                    // First command: STDIN from socket, STDOUT to pipe
                    dup2(cli_sock, STDIN_FILENO);
                    dup2(pipes[0][1], STDOUT_FILENO);
                } else if (i == clist->num - 1) 
                {
                    // Last command: STDIN from pipe, STDOUT/STDERR to socket
                    dup2(pipes[i - 1][0], STDIN_FILENO);
                    dup2(cli_sock, STDOUT_FILENO);
                    dup2(cli_sock, STDERR_FILENO);
                } else 
                {
                    // Middle command: STDIN from previous pipe, STDOUT to next pipe
                    dup2(pipes[i - 1][0], STDIN_FILENO);
