...
int exec_remote_cmd_loop(char *address, int port)
{
    ...
    while (1) 
    {
        ...
        int send_len = strlen(cmd_buff) + 1; // +1 for the null character
        ...
        while (1) 
        {
            ...
            {
                connection_closed = 1;  // Server closed connection
                break;
            }
            ...
            if (is_eof) 
            {
                printf("%.*s", (int)(io_size - 1), rsp_buff);  // Exclude EOF char
                break;
            } else 
            {
                printf("%.*s", (int)io_size, rsp_buff);  // Print full buffer
            }
            fflush(stdout);
        }
        ...
        {
            break;  // End of this command’s response
        }
    }
